...
#define ASSERT_INFO_FILE_ID FILEID__1986BE9X_DMA_C
...
{
  DMA->CFG = 0;                           /* Master Enable Off */
  DMA->CTRL_BASE_PTR = 0;                 /* Control data base pointer */
  DMA->CHNL_SW_REQUEST = 0;               /* Disable all sw requests */
  DMA->CHNL_USEBURST_CLR = 0xFFFFFFFF;    /* Disable burst mode */
  DMA->CHNL_REQ_MASK_CLR = 0xFFFFFFFF;    /* Clear mask request */
  DMA->CHNL_ENABLE_CLR = 0xFFFFFFFF;      /* Clear channel enable */
  DMA->CHNL_PRI_ALT_CLR = 0xFFFFFFFF;     /* Reset to primary data structure */
  DMA->CHNL_PRIORITY_CLR = 0xFFFFFFFF;    /* Reset to default priority */
  DMA->ERR_CLR = 0x01;                    /* Clear dma_err status */
}

/**
  * @brief  Initializes the DMA Channel DMA_ControlTable structure according to the specified
  *         parameters.
  * @param  DMA_Channel: can be 0 to 31 to select the DMA Channel.
...
void DMA_CtrlInit(uint8_t DMA_Channel, uint8_t DMA_CtrlDataType,  DMA_CtrlDataInitTypeDef* DMA_CtrlStruct)
{
  ...
  {
    uint32_t ptr = (DMA->ALT_CTRL_BASE_PTR + (DMA_Channel * sizeof(DMA_CtrlDataTypeDef)));
...
void DMA_SG_Init( uint8_t DMA_Channel, DMA_Channel_SG_InitTypeDef *DMA_SG_InitStruct)
{
  ...
  assert_param(DMA_SG_InitStruct != NULL);
  ...
  DMA->CTRL_BASE_PTR = (uint32_t)DMA_ControlTable;

  /* Primary Control Data Init */
  DMA_PriCtrlData.DMA_SourceBaseAddr = (uint32_t)(DMA_SG_InitStruct->DMA_SG_TaskArray);
  DMA_PriCtrlData.DMA_DestBaseAddr = (DMA->ALT_CTRL_BASE_PTR + (DMA_Channel * sizeof(DMA_CtrlDataTypeDef)));
  ...
  DMA->CFG = DMA_CFG_MASTER_ENABLE || DMA_SG_InitStruct->DMA_ProtCtrl;

  /* Burst mode */
  if (DMA_SG_InitStruct->DMA_UseBurst == DMA_BurstSet)
  {
    DMA->CHNL_USEBURST_SET = (1 << DMA_Channel);
  }
  else
  {
    DMA->CHNL_USEBURST_CLR = (1 << DMA_Channel);
  }

  /* Channel mask clear */
  DMA->CHNL_REQ_MASK_CLR = (1 << DMA_Channel);

  /* Enable channel */
  DMA->CHNL_ENABLE_SET = (1 << DMA_Channel);

  /* Primary - Alternate control data structure selection */
  DMA->CHNL_PRI_ALT_CLR = (1 << DMA_Channel);       /* Use Primary */

  /* Channel priority set */
  if (DMA_SG_InitStruct->DMA_Priority == DMA_Priority_High)
  {
    DMA->CHNL_PRIORITY_SET = (1 << DMA_Channel);      /* High priority */
  }
  else
  {
    DMA->CHNL_PRIORITY_CLR = (1 << DMA_Channel);      /* Default priority */
  }
}
...
void DMA_Init(uint8_t DMA_Channel, DMA_ChannelInitTypeDef* DMA_InitStruct)
{
  ...
  /* DMA configuration register */
  DMA->CFG = DMA_CFG_MASTER_ENABLE || DMA_InitStruct->DMA_ProtCtrl;
  ...
  {
    DMA->CHNL_PRI_ALT_SET = (1 << DMA_Channel);       /* Use Alternate */
  }
...
void DMA_Cmd(uint8_t DMA_Channel, FunctionalState NewState)
{
  ...
  {
    DMA->CHNL_ENABLE_CLR = (1 << DMA_Channel);
  }
}
...
void DMA_Request(uint8_t DMA_Channel)
{
  ...
  /* Set SW Request */
  DMA->CHNL_SW_REQUEST = (1 << DMA_Channel);
}
...
uint32_t DMA_GetCurrTransferCounter(uint8_t DMA_Channel, uint8_t DMA_CtrlData)
{
  ...
  {
    ptr = (DMA_CtrlDataTypeDef *)(DMA->CTRL_BASE_PTR + (DMA_Channel * sizeof(DMA_CtrlDataTypeDef)));
  }
  else
  {
    ptr = (DMA_CtrlDataTypeDef *)(DMA->ALT_CTRL_BASE_PTR + (DMA_Channel * sizeof(DMA_CtrlDataTypeDef)));
  }

  /* Read the number of remaining transfers */
  return ((ptr->DMA_Control & DMA_CONTROL_MINUS_1) + 1);
}
...
FlagStatus DMA_GetFlagStatus(uint8_t DMA_Channel, uint8_t DMA_Flag)
{
  ...
  switch(DMA_Flag)
  {
    case DMA_FLAG_DMA_ENA:
      return (FlagStatus)(DMA->STATUS & DMA_STATUS_MASTER_ENABLE);
    case DMA_FLAG_DMA_ERR:
      return (FlagStatus)(DMA->ERR_CLR & 0x01);
    case DMA_FLAG_CHNL_ENA:
      return (FlagStatus)(DMA->CHNL_ENABLE_SET & (1 << DMA_Channel));
    case DMA_FLAG_CHNL_MASK:
      return (FlagStatus)(DMA->CHNL_REQ_MASK_SET & (1 << DMA_Channel));
    case DMA_FLAG_CHNL_WAIT:
      return (FlagStatus)(DMA->WAITONREQ_STATUS & (1 << DMA_Channel));
    case DMA_FLAG_CHNL_BURST:
      return (FlagStatus)(DMA->CHNL_USEBURST_SET & (1 << DMA_Channel));
    case DMA_FLAG_CHNL_ALT:
      return (FlagStatus)(DMA->CHNL_PRI_ALT_SET & (1 << DMA_Channel));
    case DMA_FLAG_CHNL_PRIORITY:
      return (FlagStatus)(DMA->CHNL_PRIORITY_SET & (1 << DMA_Channel));
    default:
      return (FlagStatus)0;
  }
}
