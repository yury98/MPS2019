...
void ADC_Config(void)
{
  ...
  RST_CLK_PCLKcmd(((uint32_t)(1 << ((((uint32_t)(0x40088000)) >> 15) & 0x1F))), ENABLE);
  ...
  ADC_InitStructure.ADC_SynchronousMode      = (((uint32_t)0x0) << 16);
  
 
  ADC_InitStructure.ADC_TempSensor           = (((uint32_t)0x1) << 17);
  ADC_Init(&ADC_InitStructure);

  ADCx_StructInit(&ADCx_InitStructure);

   
  ADCx_InitStructure.ADC_SamplingMode     = (((uint32_t)0x1) << 3);



  ADCx_InitStructure.ADC_ChannelNumber    = ((uint32_t)0x07);
...
void ADC_Temp_Sensor_Config(void)
{
  ...
  ADC_StructInit(&ADC_InitStructure);
  ADC_InitStructure.ADC_TempSensor           = (((uint32_t)0x1) << 17);
  ADC_InitStructure.ADC_TempSensorAmplifier  = (((uint32_t)0x1) << 18);
  ADC_InitStructure.ADC_TempSensorConversion = (((uint32_t)0x1) << 19);

  ADC_Init (&ADC_InitStructure);
  ADCx_InitStructure.ADC_SamplingMode     = (((uint32_t)0x1) << 3);
  ADCx_InitStructure.ADC_ChannelNumber    = ((uint32_t)0x1F);
  ADCx_InitStructure.ADC_IntVRefSource    = ((uint32_t)0x1);
  ADCx_InitStructure.ADC_Prescaler        = (((uint32_t)0xF) << 12);
