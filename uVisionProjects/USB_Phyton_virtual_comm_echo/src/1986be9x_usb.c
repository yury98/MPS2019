...
void USB_BRGInit(const USB_Clock_TypeDef* USB_Clock_InitStruct)
{
  ...
  tmpreg |= (USB_Clock_InitStruct->USB_PLLUSBMUL << RST_CLK_PLL_CONTROL_PLL_USB_MUL_OFFS);
...
USB_Version_TypeDef USB_GetHSVR(void)
{
  ...
  s.USB_Version  = tmpreg & USB_HSVR_VERSION_MASK;
  s.USB_Revision = (tmpreg & USB_HSVR_REVISION_MASK) >> USB_HSVR_REVISION_OFFS;
