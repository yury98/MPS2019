...
void RST_CLK_GetClocksFreq(RST_CLK_FreqTypeDef* RST_CLK_Clocks)
{
  ...
  if (*(__IO uint32_t *) CPU_C2_SEL_BB != 0)
  {
    ...
    pll_mul = ((RST_CLK->PLL_CONTROL >> RST_CLK_PLL_CONTROL_PLL_CPU_MUL_OFFS) & (uint32_t)0x0F) + 1;
    cpu_c2_freq *= pll_mul;
  }

  /*Select CPU_CLK from HSI, CPU_C3, LSE, LSI cases */
  switch ((RST_CLK->CPU_CLOCK >> RST_CLK_CPU_CLOCK_HCLK_SEL_OFFS) & (uint32_t)0x03)
  {
    case 0 :
      /* HSI */
    ...
    case 1 :
      ...
      if (( RST_CLK->CPU_CLOCK >> RST_CLK_USB_CLOCK_CPU_C3_SEL_OFFS & (uint32_t)0x08) == 0x00)
      {
        cpu_c3_freq = cpu_c2_freq;
      }
      else
      {
        cpu_c3_freq = cpu_c2_freq/(1 << ((RST_CLK->CPU_CLOCK >> RST_CLK_USB_CLOCK_CPU_C3_SEL_OFFS & (uint32_t)0x07) + 1));
      }
  ...
  if (*(__IO uint32_t *) USB_C2_SEL_BB != 0)
  {
    ...
    pll_mul = ((RST_CLK->PLL_CONTROL >> RST_CLK_PLL_CONTROL_PLL_USB_MUL_OFFS) & (uint32_t)0x0F) + 1;
    usb_c2_freq *= pll_mul;
  }
  ...
  hsi_c1_freq = HSI_Value/((RST_CLK->RTC_CLOCK >> RST_CLK_RTC_CLOCK_HSI_SEL_OFFS & (uint32_t)0x0F) + 1);
  RST_CLK_Clocks->RTCHSI_Frequency = hsi_c1_freq;

  /* Compute RTCHSI_CLK frequency */
  hse_c1_freq = HSE_Value/((RST_CLK->RTC_CLOCK >> RST_CLK_RTC_CLOCK_HSE_SEL_OFFS & (uint32_t)0x0F) + 1);
  ...
  switch ((RST_CLK->ADC_MCO_CLOCK >> RST_CLK_ADC_MCO_CLOCK_ADC_C1_SEL_OFFS) & (uint32_t)0x03)
  {
    case 0 :
      /* CPU_C1 */
  ...
  switch ((RST_CLK->ADC_MCO_CLOCK >> RST_CLK_ADC_MCO_CLOCK_ADC_C2_SEL_OFFS) & (uint32_t)0x03)
  {
    case 0 :
      /* LSE */
  ...
  adc_c3_freq = adc_c2_freq /((RST_CLK->ADC_MCO_CLOCK >> RST_CLK_ADC_MCO_CLOCK_ADC_C3_SEL_OFFS & (uint32_t)0x0F) + 1);
