...
void Can2PinCfg(void)
{
    ...
    PortInit.PORT_SPEED = PORT_SPEED_MAXFAST;
    PortInit.PORT_MODE = PORT_MODE_DIGITAL;
    /* Configure PORTF pins 2 (CAN2_RX) as input */
    PortInit.PORT_OE = PORT_OE_IN;
    PortInit.PORT_Pin = PORT_Pin_2;
    PORT_Init(PORTF, &PortInit);
    /* Configure PORTF pins 3 (CAN2_TX) as output */
    PortInit.PORT_OE = PORT_OE_OUT;
...
void Can2Setup(int rate)
{
    ...
    /* Enables the CPU_CLK clock on CAN2 */
	RST_CLK_PCLKcmd(RST_CLK_PCLK_CAN2, ENABLE);
	/* */
	RST_CLK->CAN_CLOCK = 0x02000000;
	/* Timing setup */
	if (rate == 1) CAN2->BITTMNG = 1 << 27 | (3 << 25) | ((5 - 1) << 22) | ((5 - 1) << 19) | ((2 - 1) << 16) | 4; //125
	if (rate == 2) CAN2->BITTMNG = 1 << 27 | ((2-1)<<25) | ((2-1)<<22) | ((3-1)<<19) | ((2-1)<<16) | (2-1); //500
	/* TX buffer 1*/
  	CAN2->BUF_01_CON    = 0x00000000;
	CAN2->BUF_01_MASK   = 0x00000000;
	CAN2->BUF_01_FILTER = 0x00000000;	
	/* RX buffer 2*/
  	CAN2->BUF_02_CON    = 0x00000003;
	CAN2->BUF_02_ID     = 0x00000000;
	CAN2->BUF_02_DLC    = 0x00000000;
	CAN2->BUF_02_DATAL  = 0x00000000;
	CAN2->BUF_02_DATAH  = 0x00000000;
	CAN2->BUF_02_MASK   = 0x00000000;
	CAN2->BUF_02_FILTER = 0x00000000;
	/* Enable CAN2 */
	CAN2->CONTROL = 0x00000001;
}
void Tim1Setup(void)
{
...
DataH = 0x55555555;
	...
	while(1)
	{
		...
		} while ((CAN2->BUF_02_DLC & 0x00000008) != 0x00000008);
		DataL = CAN2->BUF_02_DATAL;
		DataH = CAN2->BUF_02_DATAH;
	    //clear RX buf
	  	CAN2->BUF_02_CON    = 0x00000000;
		CAN2->BUF_02_ID     = 0x00000000;
		CAN2->BUF_02_DLC    = 0x00000000;
		CAN2->BUF_02_DATAL  = 0x00000000;
		CAN2->BUF_02_DATAH  = 0x00000000;
		//send packet
		CAN2->BUF_01_ID     = 0x15AA0067;
		CAN2->BUF_01_DLC    = 0x8 | (1<<9) | (1<<11) | (1<<12);
		CAN2->BUF_01_DATAL  = DataL;
		CAN2->BUF_01_DATAH  = DataH;
		CAN2->BUF_01_CON    = 0x1 | (1<<5);
		while ((CAN2->BUF_01_CON & 0x00000020) != 0x00000000);
		CAN2->BUF_01_CON    = 0x00000000;//disable TX buffer
		CAN2->BUF_02_CON    = 0x00000003;//disable RX buffer
	}
exitC1: 	
	/* Prepare HSI clk */
...
LedStart:
	...
	BlinkLED1(20,100000,LED1);
	BlinkLED1(20,100000,LED2);
...
char s1;
	...
	{
		//select
		if (PORT_ReadInputDataBit(PORTC,PORT_Pin_2) == 0)
		{
			for (i=0;i<200000;i++){};
			if (MenuMainItem == 0)
			{
		...
		//up
		if (PORT_ReadInputDataBit(PORTB,PORT_Pin_5) == 0)
		{
			for (i=0;i<200000;i++){};
		...
		//down
		if (PORT_ReadInputDataBit(PORTE,PORT_Pin_1) == 0)
		{
			for (i=0;i<200000;i++){};
		...
		//right
		if (PORT_ReadInputDataBit(PORTB,PORT_Pin_6) == 0)
		{
			for (i=0;i<200000;i++){};
		}
		else
