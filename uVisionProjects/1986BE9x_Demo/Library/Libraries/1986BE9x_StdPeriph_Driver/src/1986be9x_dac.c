...
/* Includes ------------------------------------------------------------------*/
#include "1986BE9x_dac.h"
#include "1986BE9x_config.h"

#define ASSERT_INFO_FILE_ID FILEID__1986BE9X_DAC_C
...
void DAC_Init(uint32_t SyncMode, uint32_t DAC1_Ref, uint32_t DAC2_Ref)
{
  ...
  tmpreg_CFG = DAC->CFG;
  tmpreg_CFG &= ~(DAC_CFG_Cfg_SYNC_A | DAC_CFG_Cfg_M_REF0 | DAC_CFG_Cfg_M_REF1);
...
void DAC1_Cmd(FunctionalState NewState)
{
  ...
  if (NewState != DISABLE)
  {
    ...
    tmpreg_CFG |= DAC_CFG_Cfg_ON_DAC0;
  }
  else
  {
...
void DAC2_Cmd(FunctionalState NewState)
{
  ...
  if (NewState != DISABLE)
  {
    ...
    tmpreg_CFG |= DAC_CFG_Cfg_ON_DAC1;
  }
  else
  {
