...
#define ASSERT_INFO_FILE_ID FILEID__1986BE9X_POWER_C
...
#define POWER_OFFSET                (POWER_BASE - PERIPH_BASE)
...
#define POWER_BB(SFR, BIT)          BB_ADDR(POWER_TypeDef, POWER_OFFSET, SFR, BIT)
...
{
  POWER -> PVDCS = (uint32_t) 0x00000000;
}
...
void POWER_DUccMode(uint32_t DUccMode)
{
  ...
  tmpreg  = BKP -> REG_0E & (uint32_t) (~DUcc_Mask);
...
void POWER_DUccTrim(uint32_t DUccTrim)
{
  ...
  /* Clear POWER_REG0E[5:0] bits */
  tmpreg  = BKP -> REG_0E & (uint32_t) (~DUccTrim_Mask);
  /* Set POWER_REG0E[5:0] bits according to DUcc mode */
...
FlagStatus POWER_GetFlagStatus(uint32_t POWER_FLAG)
{
  ...
  if ((POWER->PVDCS & POWER_FLAG) != (uint32_t)RESET)
  {
    status = SET;
  }
  else
...
void POWER_ClearFlag(uint32_t POWER_FLAG)
{
  ...
  POWER->PVDCS &= (uint32_t) ~POWER_FLAG;
}
...
void POWER_PVD_ITconfig(uint32_t POWER_IT, FunctionalState NewState)
{
  ...
  if (NewState != DISABLE)
  {
    POWER->PVDCS |= POWER_IT;
  }
  else
  {
    POWER->PVDCS &= (uint32_t)~POWER_IT;
  }
}
...
void POWER_PVD_INVconfig(uint32_t POWER_INV, FunctionalState NewState)
{
  ...
  if (NewState != DISABLE)
  {
    POWER->PVDCS |= POWER_INV;
  }
  else
  {
    POWER->PVDCS &= (uint32_t)~POWER_INV;
  }
}
