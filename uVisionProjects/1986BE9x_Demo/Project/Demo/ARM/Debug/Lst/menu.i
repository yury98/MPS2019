...
static tMenu psPrevMenu[4];
static tMenu psCurrentMenu;

 
struct sMenuItem TimeMenuItems[] = {{"Adjust", TimeAdjustFunc, 0},
                                    {"Show", TimeShowFunc, 0},
                                    {"Return", ReturnFunc, 0}};
struct sMenu TimeMenu = {"Time", TimeMenuItems, (sizeof(TimeMenuItems) / sizeof(*(TimeMenuItems)))};

struct sMenuItem DateMenuItems[] = {{"Adjust", DateAdjustFunc, 0},
                                    {"Show", DateShowFunc, 0},
                                    {"Return", ReturnFunc, 0}};
struct sMenu DateMenu = {"Date", DateMenuItems, (sizeof(DateMenuItems) / sizeof(*(DateMenuItems)))};

struct sMenuItem AlarmMenuItems[] = {{"Adjust", AlarmAdjustFunc, 0},
                                     {"Show", AlarmShowFunc, 0},
                                     {"Return", ReturnFunc, 0}};
struct sMenu AlarmMenu = {"Alarm", AlarmMenuItems, (sizeof(AlarmMenuItems) / sizeof(*(AlarmMenuItems)))};

struct sMenuItem STOPMenuItems[] = {{"Exit on RTC Alarm", STOPMode_RTCAlarm, 0},
                                    {"Return", ReturnFunc, 0}};
struct sMenu STOPMenu = {"STOP Mode", STOPMenuItems, (sizeof(STOPMenuItems) / sizeof(*(STOPMenuItems)))};

struct sMenuItem STANDBYMenuItems[] = {

																			 {"Exit on Wakeup", STANDBYMode_WAKEUP, 0},

																			 {"Exit on RTC Alarm", STANDBYMode_RTCAlarm, 0},
                                       {"Return", ReturnFunc, 0}};
struct sMenu STANDBYMenu = {"STANDBY Mode", STANDBYMenuItems, (sizeof(STANDBYMenuItems) / sizeof(*(STANDBYMenuItems)))};

struct sMenuItem UARTMenuItems[] = {{"With FIFO", UARTwFIFOFunc, 0},
                                    {"Without FIFO", UARTFunc, 0},
                                    {"Return", ReturnFunc, 0}};
struct sMenu UARTMenu = {"UART", UARTMenuItems, (sizeof(UARTMenuItems) / sizeof(*(UARTMenuItems)))};

struct sMenuItem RTCMenuItems[] = {{"Time", IdleFunc, &TimeMenu},
                                   {"Date", IdleFunc, &DateMenu},
                                   {"Alarm", IdleFunc, &AlarmMenu},
                                   {"Return", ReturnFunc, 0}};
struct sMenu RTCMenu = {"Real Time Clock", RTCMenuItems, (sizeof(RTCMenuItems) / sizeof(*(RTCMenuItems)))};

struct sMenuItem LowPowerMenuItems[] = {{"STOP", IdleFunc, &STOPMenu},
                                        {"STANDBY", IdleFunc, &STANDBYMenu},
                                        {"Return", ReturnFunc, 0}};
struct sMenu LowPowerMenu = {"Low Power Mode", LowPowerMenuItems, (sizeof(LowPowerMenuItems) / sizeof(*(LowPowerMenuItems)))};

 
struct sMenuItem TextMenuItems[] = {{"Font", FontFunc, 0},
                                    {"Style", StyleFunc, 0},
                                    {"Book", BookFunc, 0},
                                    {"Return", ReturnFunc, 0}};
struct sMenu TextMenu = {"Text", TextMenuItems, (sizeof(TextMenuItems) / sizeof(*(TextMenuItems)))};

struct sMenuItem GraphicMenuItems[] = {{"Elements", ElementsFunc, 0},
                                       {"Indicators", IndicatorsFunc, 0},
                                       {"Return", ReturnFunc, 0}};
struct sMenu GraphicMenu = {"Graphics", GraphicMenuItems, (sizeof(GraphicMenuItems) / sizeof(*(GraphicMenuItems)))};

struct sMenuItem LEDsMenuItems[] = {{"On", LightsOnFunc, 0},
                                    {"Return", ReturnFunc, 0}};
struct sMenu LEDsMenu = {"LEDs", LEDsMenuItems, (sizeof(LEDsMenuItems) / sizeof(*(LEDsMenuItems)))};

struct sMenuItem CommMenuItems[] = {{"UART", IdleFunc, &UARTMenu},
                                    {"USB. Virtual COM", VCOMFunc, 0},
                                    {"Return", ReturnFunc, 0}};
struct sMenu CommMenu = {"Communication", CommMenuItems, (sizeof(CommMenuItems) / sizeof(*(CommMenuItems)))};

struct sMenuItem OtherMenuItems[] = {{"Thermometer", TSENSORFunc, 0},
                                     {"Real Time Clock", IdleFunc, &RTCMenu},
                                     {"Low Power", IdleFunc, &LowPowerMenu},
                                     {"Return", ReturnFunc, 0}};
struct sMenu OtherMenu = {"Other devices", OtherMenuItems, (sizeof(OtherMenuItems) / sizeof(*(OtherMenuItems)))};
...
struct sMenuItem LCDMenuItems[] = {{"Text", IdleFunc, &TextMenu},
                                   ...
                                   {"Return", ReturnFunc, 0}};
struct sMenu LCDMenu = {"Display", LCDMenuItems, (sizeof(LCDMenuItems) / sizeof(*(LCDMenuItems)))};

struct sMenuItem DevMenuItems[] = {{"Communication", IdleFunc, &CommMenu},
                                   {"Other", IdleFunc, &OtherMenu},
                                   {"Return", ReturnFunc, 0}};
struct sMenu DevMenu = {"Devices", DevMenuItems, (sizeof(DevMenuItems) / sizeof(*(DevMenuItems)))};
...
struct sMenuItem MainMenuItems[] = {
  ...
  {"About", AboutFunc, 0}};
struct sMenu MainMenu = {"Main menu", MainMenuItems, (sizeof(MainMenuItems) / sizeof(*(MainMenuItems)))};
...
void DisplayMenuItemString(uint32_t y, ucint8_t *ptr)
{
  ...
  LCD_PUTS(0, y, "                                        ");
  x = (127 - (CurrentFont->Width * strlen((const char *)ptr))) / 2;
  LCD_PUTS(x, y, ptr);
}
