...
void Date_Update(void)
{
  ...
  BKP->REG_01 = sDate.year + (sDate.month << 16) + (sDate.day << 24);
  /* Wait until last write operation on RTC registers has finished */
  BKP_RTC_WaitForUpdate();
}
...
void Date_Regulate(void)
{
  ...
  do
  {
    ...
    Tmp_day  = Tmp_day * 10 + ReadDigit(DateD_l, (sDate.day % 10), 0x9, 0x0);
    /* Read month */
    Tmp_month = ReadDigit(DateM_h, (sDate.month / 10), 0x1, 0x0);
    Tmp_month = Tmp_month * 10 + ReadDigit(DateM_l, (sDate.month % 10), 0x2, 0x0);
    /* Read year */
  ...
  BKP->REG_01 = Tmp_year + (Tmp_month << 16) + (Tmp_day << 24);
  /* Wait until last write operation on RTC registers has finished */
  BKP_RTC_WaitForUpdate();
}
...
void Alarm_Display()
{
  uint32_t AlarmVar = BKP->RTC_ALRM;
...
uint32_t Alarm_Regulate(void)
{
  ...
  BKP->REG_02 = Alarm_HH*3600 + Alarm_MM*60 + Alarm_SS;
