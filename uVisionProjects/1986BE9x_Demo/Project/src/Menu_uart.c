...
void SenderFuncFIFODemo(void)
{
  if (error_flag == 0)
  {
    for (; ((UART_GetFlagStatus (UART1, UART_FLAG_TXFF) == RESET) && (send_buffer_pos < SEND_BUFFER_SIZE)); send_buffer_pos++)
    {
      UART_SendData(UART1, SendBuffer[send_buffer_pos]);
    }
  }
}
...
void ReceiverFuncFIFODemo(void)
{
  ...
  if (error_flag == 0)
  {
    for (; ((UART_GetFlagStatus(UART1, UART_FLAG_RXFE) == RESET) && (receive_buffer_pos < RECEIVE_BUFFER_SIZE) && (error_flag == 0));
             receive_buffer_pos++)
    {
...
void UARTwFIFOFunc(void) {
  ...
  UART_Init (UART1,&UARTInitStructure);
  ...
  UART_ITConfig (UART1, UART_IT_TX | UART_IT_RX, ENABLE);

  /* Adjust sender and receiver interrupt levels */
  UART_DMAConfig (UART1, FIFO_IT_LVL_TO_UART_IT_CONST(RECEIVE_FIFO_LVL),
                         FIFO_IT_LVL_TO_UART_IT_CONST(SEND_FIFO_LVL));
  /* Enable DMA interrupts */
  UART_DMACmd(UART1, UART_DMA_TXE | UART_DMA_RXE, ENABLE);
...
void UARTFunc(void) {
  ...
  for (; key != SEL; key = GetKey())
  {
    if ((key != NOKEY) && (key != MULTIPLE))
    {
      ...
      while (UART_GetFlagStatus (UART1, UART_FLAG_RXFF) == SET)
      {
      ...
      for (pdata = SendData[key - 1], i = 0; ((*pdata != 0) && (error_flag == 0)); pdata++, i++)
      {
        ...
        while (UART_GetFlagStatus (UART1, UART_FLAG_TXFF) == SET);
        while (UART_GetFlagStatus (UART1, UART_FLAG_BUSY) == SET);
